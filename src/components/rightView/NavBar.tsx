...
const NavBar = () => {
  ...
  let optionDiv: any = useRef();
  let hamburgerDiv: any = useRef();
  const data: any = [
    {
      linkName: "Tableau de bord",
      icon: <MdOutlineDashboard color={iconColor} />,
      active: false,
    },
    {
      linkName: "Paie",
      icon: <CiBank color={iconColor} />,
      active: false,
    },
    {
      linkName: "Accompte",
      icon: <IoWalletOutline color={iconColor} />,
      active: false,
    },
    {
      linkName: "Absences",
      icon: <FaRegCalendarXmark color={iconColorActive} />,
      active: true,
    },

    {
      linkName: "Notes de frais",
      icon: <GrNotes color={iconColor} />,
      active: false,
    },
    {
      linkName: "Etablissemnt",
      icon: <MdOutlineMapsHomeWork color={iconColor} />,
      active: false,
    },
    {
      linkName: "Mes documents",
      icon: <GoFileDirectory color={iconColor} />,
      active: false,
    },
  ];
  useEffect(() => {
  ...
  return (
    <div>
      <div className="flex justify-center mt-8">
        <div className="bg-white dark:bg-slate-800 p-1 px-2 lg:p-3 rounded-lg flex justify-between items-center w-full ">
            ...
            <div className="block lg:hidden">
              ...
              <div>
                {isOpen ? (
                  <div ref={optionDiv}>
                    <div
                      className={`right-8 dark:bg-slate-800 mt-10 gap-5 flex flex-col z-10 absolute bg-white px-4 py-2 rounded-lg`}
                    >
                      {data.map((item: any, index: number) => (
                        <div key={index} className="flex gap-3 items-center">
                          <div>{item.icon}</div>
